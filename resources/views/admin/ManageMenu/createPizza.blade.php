...
@section('content')
 <div class="container">
    <div class="row">
        <div class="col">
            <h4 class="mb-2">Add Pizza <i class="fa-solid fa-pizza-slice"></i></h4>
        ...
        <form action="{{ route('adminStorePizza') }}" class="p-3 rounded" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="mt-2 row">
                <div class="col-4">
                ...
                <div class="col-6 offset-1">
                    <input type="text" name="name" id="" placeholder="Enter Pizza Name..." class="form-control @error('name') is-invalid @enderror mb-2" value="{{ old('name') }}">
                    @error('name')
                        <small class=" invalid-feedback">{{ $message }}</small>
                    @enderror
                    ...
                    <select name="categoryId" id="" class="form-control @error('categoryId') is-invalid @enderror mb-2">
                        ...
                        @foreach ($categories as $item)
                            <option value="{{ $item->id }}" {{ old('categoryId') == $item->id ? 'selected' : '' }}
                                @if($item->id == 3 || $item->id == 4 || $item->id == 8 || $item->id == 9) disabled @endif>{{ $item->name }}</option>
                        @endforeach
                    </select>
                    @error('categoryId')
                        <small class=" invalid-feedback">{{ $message }}</small>
